...
start = time.time()
sql_query = text("INSERT INTO '07032004' (id,data,sender_id) VALUES (2,'heeeeeeeeha',1);")
session.execute(sql_query)
session.commit()
final = time.time()
print("Insert query time with sqlscript : "+str((final-start)*1000))

sql_query = text("SELECT * from '07032004';")
result = session.execute(sql_query).fetchall()
session.commit()
print("id is "+str(result[0][0]))
print(result)
data = session.query(channel_class).filter_by(id=2).first()
print(data.user.name)
final = time.time()
